...
;
--Write a query to find the total revenue, total quantity sold, and the overall average price per unit across the entire dataset.
SELECT 
...
--Retrieve all transactions for Female customers where the total_sale amount was greater than 1,000.
SELECT 
    transactions_id,
    sale_date,
    customer_id,
...
ORDER BY 1
;
--Calculate the total net profit for each product category. Net profit is calculated as total_sale - cogs.
SELECT 
    category,
...
-- Q8. Write a SQL query to find the top 8 customers based on the highest total sales
SELECT 
    customer_id,
...
--Q11. Group customers based on their age into generations (e.g., Gen Z, Millennials, Gen X, Boomers) and calculate the total sales, average order value (AOV), and total quantities purchased for each group to find our most valuable demographic.
SELECT 
    CASE 
...
-- Q12. Calculate the month-over-month (MoM) growth rate of total sales across the entire dataset to track revenue momentum.
...
--For each product category, determine its total revenue contribution percentage (market share) relative to the company's total revenue, and rank them.
SELECT 
    category,
...
--Perform a data validation check to find any rows where the listed total_sale does not equal quantiy * price_per_unit. (Note: Your raw dataset contains a minor typo in the column name quantiy).
SELECT 
    transactions_id,
    quantity,
    price_per_unit,
